...
import static io.grpc.xds.internal.security.CommonTlsContextTestsUtil.INVALID_WC_DNS_PEM_FILE;
...
import static io.grpc.xds.internal.security.CommonTlsContextTestsUtil.VALID_WC_DNS_PEM_FILE;
...
public class XdsX509TrustManagerTest {
  ...
  public void testVerifyDnsName_succeedsForValidWildcardSanNames()
    ...
    trustManager = new XdsX509TrustManager(certContext, mockDelegate);
    X509Certificate[] certs =
            CertificateUtils.toX509Certificates(TlsTesting.loadCert(VALID_WC_DNS_PEM_FILE));
    trustManager.verifySubjectAltNameInChain(certs);
  }
  ...
  public void testVerifyDnsName_failsForInvalidWildcard_SanNames()
    ...
    trustManager = new XdsX509TrustManager(certContext, mockDelegate);
    X509Certificate[] certs =
            CertificateUtils.toX509Certificates(TlsTesting.loadCert(INVALID_WC_DNS_PEM_FILE));
    try {
      trustManager.verifySubjectAltNameInChain(certs);
      fail("no exception thrown");
    } catch (CertificateException expected) {
